...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [conferences, posts, proceedings] = await Promise.all([
    ...
    db.proceeding.findMany({ where: { status: "PUBLISHED", publishedAt: { not: null } }, select: { slug: true, updatedAt: true } }),
  ]);
  ...
  return [
    ...
    ...proceedings.map(item => ({ url: `${base}/proceedings/${item.slug}`, lastModified: item.updatedAt })),
  ];
}
