...
export async function createSubmissionAction(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const submission = await db.submission.create({ data: { ...parsed.data, keywords: parsed.data.keywords || null, trackId: parsed.data.trackId || null, ownerId: user.id, authors: { create: { name: user.name || user.email, email: user.email, corresponding: true } } } });
...
export async function addAuthorAction(submissionId: string, _: ActionState, formData: FormData): Promise<ActionState> {
  ...
  await db.submissionAuthor.create({ data: { submissionId, ...parsed.data, affiliation: parsed.data.affiliation || null, country: parsed.data.country || null, sortOrder: count } });
...
export async function submitSubmissionAction(submissionId: string) {
  ...
  await db.submission.update({ where: { id: submissionId }, data: { status: "SUBMITTED", submittedAt: new Date() } }); await audit("SUBMISSION_SUBMITTED", user.id, "Submission", submissionId); redirect("/workspace");
}
export async function withdrawSubmissionAction(submissionId: string) {
  const user = await requireAccount(); const result = await db.submission.updateMany({ where: { id: submissionId, ownerId: user.id, status: { in: ["SUBMITTED", "UNDER_REVIEW", "REVISION_REQUESTED"] } }, data: { status: "WITHDRAWN", withdrawnAt: new Date() } });
  if (!result.count) throw new Error("This submission cannot be withdrawn."); await audit("SUBMISSION_WITHDRAWN", user.id, "Submission", submissionId); redirect("/workspace");
}
...
export async function recordDecisionAction(submissionId: string, _: ActionState, formData: FormData): Promise<ActionState> {
  ...
  await db.$transaction([db.submissionDecision.create({ data: { submissionId, decidedById: manager.id, ...parsed.data } }), db.submission.update({ where: { id: submissionId }, data: { status: statusForDecision(parsed.data.type) } })]); await audit("SUBMISSION_DECIDED", manager.id, "Submission", submissionId, { decision: parsed.data.type }); revalidatePath(`/workspace/manage/submissions/${submissionId}`); return { ok: true, message: "Decision recorded and visible to the author." };
}
