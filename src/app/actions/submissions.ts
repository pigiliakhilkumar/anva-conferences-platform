...
export async function submitReviewAction(assignmentId: string, _: ActionState, formData: FormData): Promise<ActionState> {
  ...
  await db.$transaction([db.review.create({ data: { assignmentId, recommendation: parsed.data.recommendation, score: parsed.data.score === "" ? null : parsed.data.score ?? null, authorComments: parsed.data.authorComments, confidentialComments: parsed.data.confidentialComments || null } }), db.reviewAssignment.update({ where: { id: assignmentId }, data: { status: "COMPLETED", completedAt: new Date() } })]); const owner = await db.submission.findUnique({ where: { id: assignment.submissionId }, select: { ownerId: true, referenceNumber: true } }); if (owner) await notify(owner.ownerId, "REVIEW_SUBMITTED", "Peer review submitted", `A review has been submitted for ${owner.referenceNumber}.`); await audit("REVIEW_SUBMITTED", user.id, "ReviewAssignment", assignmentId); revalidatePath(`/workspace/reviews/${assignmentId}`); return { ok: true, message: "Review submitted. It is now read-only." };
}
