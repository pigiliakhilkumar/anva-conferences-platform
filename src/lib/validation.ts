...
export const submissionSchema = z.object({
  ...
  kind: z.enum(["ABSTRACT", "FULL_PAPER", "POSTER", "WORKSHOP_PROPOSAL"]), trackId: z.string().cuid().optional().or(z.literal("")),
});
export const authorSchema = z.object({ name: z.string().trim().min(2).max(150), email: z.string().trim().toLowerCase().email().max(254), affiliation: z.string().trim().max(250).optional().or(z.literal("")), country: z.string().trim().max(100).optional().or(z.literal("")), corresponding: z.preprocess(v => v === "on" || v === true, z.boolean()) });
...
export const decisionSchema = z.object({ type: z.enum(["ACCEPT", "REVISION_REQUIRED", "REJECT"]), comments: z.string().trim().min(10).max(10000) });
